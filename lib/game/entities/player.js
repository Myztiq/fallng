  ...
  EntityPlayer = ig.Entity.extend({
    ...
    onLanded: function() {
      ...
      if (!this.isLanded)
      {
        ...
        var totalBgHeight = (bg.data.length * 800);
        var totalBgHeightPixels = totalBgHeight * bg.distance;
        this.pos.y = totalBgHeightPixels - viewPortHeight;
        this.isLanded = true; 
        console.log("Landed" + this.pos.y);
      }
    },
    update: function() {
      if ( this.isLanded )
      ...
      var imageHeightPixels = (bg.data.length * 800); // pixels
      var imageHeightGame = (imageHeightPixels * bg.distance) ;

      if (this.pos.y >= imageHeightGame - viewPortHeight )
      {
        if ( !this.isLanded )
        {
          // console.log(imageHeightGame);
          this.onLanded();
        }
        return;
      }
